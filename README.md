...
live instance loads. The current release is `v1.4.0`.

1. Pick the script you want, e.g. `example.js`.
2. Reference its jsDelivr URL pinned to a tag:

   ```
   https://cdn.jsdelivr.net/gh/maxrugen/globaljs@v1.4.0/example.js
   ```

3. Inject it via the platform's Global JS field, e.g.:

   ```html
   <script src="https://cdn.jsdelivr.net/gh/maxrugen/globaljs@v1.4.0/example.js" defer></script>
   ```

4. To roll out a change, cut a new tag (`git tag v1.4.0 && git push --tags`) and bump the version
